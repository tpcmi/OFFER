...
通常类的成员函数不改变类的数据成员的时候，都会加上const以提高程序的可读性和可靠性，

**顶层const**：指针本身是一个常量，在拷贝构造时不能被改变(const 在`*`右侧)，必须初始化

**底层const**：指针所指的对象是一个常量，该指针可以改变（const在`*`左侧）

```c++
const int *const p=&x;    //靠左的是底层const，靠右的是顶层const；
```
...
宏展开是在预编译阶段完成的，这个阶段没有任何计算，而是当作字符串直接替换
...
当使用`=`时，如果右边恰好为类的参数构造器的参数类型，会调用普通类中自带的隐式的构造函数，有时候会造成一定的隐患，此时在构造函数前添加`explicit`可以避免
