...
堆是按需申请，new动态分配，而且需要主动delete；

栈是程序运行时自动拥有的一小块内存，在编译时期由编译器参数决定，用于 ==局部变量== 的存放或者 ==函数调用栈== 的保存，栈的空间有限制，一旦局部变量申请过多（例如开个超大数组），或者函数调用太深（例如递归太多次），那么就会导致栈溢出（Stack Overflow），操作系统这时候就会直接把程序杀掉

### 自由存储区

就是那些由malloc等分配的内存块，和堆十分相似，用free来释放
